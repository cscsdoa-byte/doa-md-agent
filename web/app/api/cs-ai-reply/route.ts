...
- 인사말 (예: "안녕하세요, 고객님 ^^")
- **이모티콘**(키보드 문자만): ^^ ~ ㅠ ㅠㅠ ㅎ ㅎㅎ ㅋ ㅋㅋ ! ?
- **이모지(😊🎉✅⚠️📌🚨 등 Unicode 그림) 절대 사용 X** — 카톡 답변용
- 문장 길이·줄바꿈
...
// Unicode 이모지 자동 제거 (Claude 가 system prompt 따르지 않을 때 보강).
// 키보드 이모티콘(^^ ~ ㅋ ㅎ ㅠ)은 보존, 그림 이모지만 제거.
function stripEmoji(s: string): string {
  if (!s) return s;
  // Emoji 주요 범위 (Misc Symbols, Emoticons, Transport, Supplemental Symbols, Flags)
  // 화살표/체크/경고 같은 기호 그림(✅ ⚠️ ❌ 📌 🚨 등)도 포함
  return s
    .replace(/[\u{1F300}-\u{1FAFF}]/gu, "")     // Misc Symbols & Pictographs, Emoticons, Transport, Supplemental
    .replace(/[\u{2600}-\u{27BF}]/gu, "")        // Misc Symbols + Dingbats (✅⚠️❌ 등)
    .replace(/[\u{1F1E6}-\u{1F1FF}]/gu, "")      // Regional indicators (flags)
    .replace(/️/gu, "")                     // Variation Selector-16 (이모지 표시)
    .replace(/‍/gu, "")                     // Zero-width joiner
    .replace(/[ \t]{2,}/g, " ")                   // 정리 후 다중 공백 → 1개
    .trim();
}
  ...
  if (productNames.size > 0) {
    ...
    for (const product of productNames) {
      ...
      if (k) {
        ...
        if (k.manual_notes) lines.push(`  · 🔒 직접 입력 메모 (반드시 반영): ${k.manual_notes}`);
...
- **이모지 절대 금지** — 😊 🎉 ✅ ⚠️ ❌ 💡 📌 🚨 등 Unicode 그림문자 X.
  대신 **이모티콘만 허용**: ^^ ~ ㅠ ㅠㅠ ㅎ ㅎㅎ ㅋ ㅋㅋ
  ✗ "감사합니다 😊" → ✓ "감사합니다 ^^"
  ✗ "✅ 정상가: 28,900원" → ✓ "정상가 28,900원" (그냥 텍스트만)
...
JSON 형식으로 답변 옵션 3개와 후속 액션을 작성해주세요.`;
  ...
  try {
    ...
    // 이모지 후처리 — Claude 가 system prompt 따르지 않을 때 안전망
    ...
    return NextResponse.json({
      analysis,
      analysis_summary: stripEmoji(parsed.analysis_summary ?? ""),
      replies: cleanedReplies,
      follow_up_actions: (parsed.follow_up_actions ?? []).map(stripEmoji),
      escalation: stripEmoji(parsed.escalation ?? ""),
